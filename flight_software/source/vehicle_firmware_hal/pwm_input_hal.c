...
void setup_timer15_input_capture(void)
{
	...
	timer_set_prescaler(TIM15, 63); // For 1 MHz Timer clock, 1 counts/us
	timer_enable_counter(TIM15);

	gpio_set_output_options(GPIOF, GPIO_OTYPE_PP, GPIO_OSPEED_50MHZ, GPIO9);
	gpio_mode_setup(GPIOF, GPIO_MODE_AF, GPIO_PUPD_NONE, GPIO9);
	gpio_set_af(GPIOF, GPIO_AF3, GPIO9);

	timer_ic_set_input(TIM15, TIM_IC1, TIM_IC_IN_TI1);
	timer_ic_set_input(TIM15, TIM_IC2, TIM_IC_IN_TI1);

	timer_ic_set_polarity(TIM15, TIM_IC1, TIM_IC_RISING);
	timer_ic_set_polarity(TIM15, TIM_IC2, TIM_IC_FALLING);

	timer_slave_set_trigger(TIM15, TIM_SMCR_TS_IT1FP1);
	timer_slave_set_mode(TIM15, TIM_SMCR_SMS_RM);

	nvic_set_priority(NVIC_TIM1_BRK_TIM15_IRQ, 2);
	nvic_enable_irq(NVIC_TIM1_BRK_TIM15_IRQ);

	timer_enable_irq(TIM15, TIM_DIER_CC1IE | TIM_DIER_CC2IE | TIM_DIER_UIE);
...
void timer15_isr_callback(void)
{
	if((TIM15_SR & TIM_SR_UIF) != 0)
	{
		timer_clear_flag(TIM15, TIM_SR_UIF);
	}
	if ((TIM15_SR & TIM_SR_CC2IF) != 0)
	{
		timer_clear_flag(TIM15, TIM_SR_CC2IF);
		current_state.duty_data[4] = TIM15_CCR2;
	}
	if ((TIM15_SR & TIM_SR_CC1IF) != 0)
	{
		timer_clear_flag(TIM15, TIM_SR_CC1IF);
		current_state.period_data[4] = TIM15_CCR1;
		/*
			Kick the dog:
		 */
		rc_tim15_watchdog_counter = RC_TIMEOUT_THRESHOLD;
		current_state.channel_states[4] = CHANNEL_VALID;
	}
}
...
void rc_input_watchdog_callback(void)
{
	...
	if(rc_tim15_watchdog_counter == 0U)
	{
		rc_tim15_watchdog_counter = RC_TIMEOUT_THRESHOLD;
		current_state.channel_states[4] = CHANNEL_INVALID;
	}
	else
	{
		--rc_tim15_watchdog_counter;
	}
}
