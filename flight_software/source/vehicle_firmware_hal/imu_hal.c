...
void imu_i2c_bus_clear(void)
{
	...
	/* Set GPIO6 and 6 (in GPIO port B) to 'output push-pull'. */
	gpio_mode_setup(GPIOB, GPIO_MODE_OUTPUT, GPIO_PUPD_NONE, GPIO6); //SCL is output
	gpio_mode_setup(GPIOB, GPIO_MODE_INPUT, GPIO_PUPD_NONE, GPIO7);	//SDA is tri-stated (Input)
	...
	/*
		Delay by 10 ms before clearing I2C pins:
	 */	
	...
	/*
		Clock out 8 high bits via SDA, to force all slaves to release I2C1 Bus:
	 */
	...
	for(j = 0U; j < 9U; ++j)
	{
		...
		{
			printf("Exiting reset loop at iteration %d\r\n", j);
			break;
		}
	}
	gpio_set(GPIOB, GPIO6);
	...
	/*
		Delay by 10 ms before re-allocating GPIO pins to I2C1 alternate 
