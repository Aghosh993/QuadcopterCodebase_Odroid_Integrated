...
std::vector<std::vector<float>> predict(std::vector<float> UAV_pos, std::vector<float> UGV_pos, std::vector<float> UGV_speed) {
	std::cout << "predict\n";
	float dist = std::sqrt((UAV_pos[0] - UGV_pos[0])*(UAV_pos[0] - UGV_pos[0]) + (UAV_pos[1] - UGV_pos[1])*(UAV_pos[1] - UGV_pos[1]));
	float t_ugv = dist;
	float t_uav = 0;
	...
	std::vector<float> speedVec_ugv = { dest[0] - dest_prev[0],dest[1] - dest_prev[1] };
	float speedVec_ugv_ang = (180 / 3.14)*std::atan2(speedVec_ugv[1], speedVec_ugv[0]); //Speed std::vector of UGV
	//std::vector<float> speedVec_uav = { -dest_prev[0] + UAV_pos[0],-dest_prev[1] + UAV_pos[1] };
	//float speedVec_uav_ang = (180 / 3.14)*std::atan2(speedVec_uav[1], speedVec_uav[0]);
	//Angle difference
	...
	float radius = 1.5;
	//Curve Calculation
	float ugv_ang = std::atan2((dest_prev[1] - dest[1]), (dest_prev[0] - dest[0]));
	...
	if (curve_style == "no_curve") {
		std::cout << "no_curve\n";
		new_path.push_back({ dest_prev[0],dest_prev[1],2,8 });
	}
	else if (curve_style == "180_curve") {
		std::cout << "180_curve\n";
		for (int i = x.size()-1; i > 0; i--) {
			new_path.push_back({ dest_prev[0] + x[i],dest_prev[1] + y[i],z[i],6 });
		}
	}
...
int landing_path_planner(std::vector<float> ugv_pos, std::vector<float> uav_pos, std::vector<float> ugv_speed) {
	...
	//ugv_pos[1] = -ugv_pos[1]; uav_pos[1] = -uav_pos[1]
